{pkgs}: let
  customRC = ''
               ...
               " Typst Configuration
            let g:ale_typst_tinymist_executable = 'tinymist'
            let g:ale_typst_tinymist_options =
            \ '--preview --preview-host 127.0.0.1 --preview-port 3141'

              " --- Typst Lifecycle Management ---
    augroup typst_preview
        autocmd!
        " Only run for real files, not scratch/preview/ALE buffers
        autocmd BufEnter *.typ if empty(&buftype) | call s:StartTypstPreview() | endif
        autocmd BufLeave,VimLeave *.typ if empty(&buftype) | call s:StopTypstPreview() | endif
    augroup END

    let s:typst_job = v:none " Use v:none instead of 0 for Job types

    function! s:StartTypstPreview()
        " Check if the job is NOT running using the proper Job-aware check
        if s:typst_job is v:none || job_status(s:typst_job) !=# 'run'
            let l:cmd = 'tinymist preview --host nixos.local:3141 --no-open ' . expand('%:p')
            " We wrap in ['sh', '-c'] to handle the full string command comfortably
            let s:typst_job = job_start(['sh', '-c', l:cmd])
            echo "Tinymist Preview Started (nixos.local:3141)"
        endif
    endfunction

    function! s:StopTypstPreview()
        " Use 'isnot' to compare objects properly without E910
        if s:typst_job isnot v:none && job_status(s:typst_job) ==# 'run'
            call job_stop(s:typst_job)
            let s:typst_job = v:none
            echo "Tinymist Preview Stopped"
        endif
    endfunction
